...
suspend fun BehaviourContext.moderateHandleUniversityUpdateRequestCloseDoneCallback(
    ...
    val commentMessage = waitTextMessage(
        SendTextMessage(
            ...
            buildEntities {
                +bold("${university.shortName} -> Закрытие запроса на изменение информации #${updateRequestId} с пометкой о выполнении") +
    ...
    databaseQuery {
        updateRequestEntity!!.apply {
            this.status = UniversityUpdateRequestStatus.ClosedAndDone
